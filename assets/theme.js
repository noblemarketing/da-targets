...
theme.Slideshow = (function() {
  ...
  function slideshow(el, sectionId) {
    ...
    var centerMode = this.$slideshow.data('center-mode');
    var centerPadding = this.$slideshow.data('center-padding');
    this.settings = {
      rtl:theme.rtl,
      accessibility: false,
      ...
      slidesToShow: centerMode === true ? 1 : 1
    };
              ...
              jQuery.getJSON(`/products/${product_handle}.js`, function(product) {
                $(product.variants).each(function(i, v) {
                  if (v.title == selectedOptions) {
                    ...
                    var compare_price = v.compare_at_price > 0 ? theme.Currency.formatMoney(v.compare_at_price, theme.moneyFormat) : '';
...
theme.addCartButton = (function(){
  ...
  $(document).on('click',buttonClass,function(e){
    ...
    var hasMultipleVariants = $this.data('has-variants');
    ...
    console.log('Button data:', {
      ...
      type: typeof hasMultipleVariants
    });
    
     // If product has multiple variants, show inline variant selector
    // Convert to number to ensure proper comparison
    var variantCount = parseInt(hasMultipleVariants, 10);
    console.log('Variant count (parsed):', variantCount);
    
    if (variantCount && variantCount > 1) {
      e.preventDefault();

      // Collection / product cards: send shoppers to the product page instead of inline variant UI.
      // Product page (Choose Option) keeps existing behavior inside .product-button-variant.
      if (!$this.closest('.product-button-variant').length && productUrl) {
        window.location.href = productUrl;
        return;
      }
      
      
      console.log('Product has variants:', hasMultipleVariants);
      console.log('Product URL:', productUrl);
      console.log('Product card:', $productCard);
      
      // Check if variant selector already exists in the button area
      var $buttonContainer = $this.closest('.product-card-cart, .product-form__item--submit, .product-card__overlay');
      if ($buttonContainer.length === 0) {
        $buttonContainer = $this.parent();
      }
      var $variantSelector = $buttonContainer.find('.inline-variant-selector');
      if ($variantSelector.length > 0) {
        console.log('Toggling existing selector');
        var $gridItem = $this.closest('.grid__item');
        
        // Toggle visibility with fade
        if ($variantSelector.is(':visible')) {
          $variantSelector.fadeOut(300, function() {
            // Remove z-index helper class when selector is hidden
            $gridItem.removeClass('variant-selector-active');
          });
        } else {
          // Add z-index helper class when selector is shown
          $gridItem.addClass('variant-selector-active');
          $variantSelector.fadeIn(300);
        }
        return;
      }
      ...
      // Construct proper product JSON URL
      var productHandle = productUrl.split('/').pop().split('?')[0]; // Remove query params
      var jsonUrl = productUrl.split('?')[0] + '.js'; // Try appending .js to the full URL first
      console.log('Product URL:', productUrl);
      console.log('Product Handle:', productHandle);
      console.log('JSON URL (method 1):', jsonUrl);
      
      // Fetch product data via AJAX with timeout and proper error handling
      $.ajax({
        url: jsonUrl,
        type: 'GET',
        dataType: 'json',
        ...
        success: function(product) {
          ...
          // Verify we have the theme function
          if (typeof theme.createInlineVariantSelector !== 'function') {
            console.error('theme.createInlineVariantSelector is not a function');
            window.location.href = productUrl;
            return;
          }
          
          // Create inline variant selector
          var selectorHtml = theme.createInlineVariantSelector(product);
          console.log('Created selector HTML');
          
          // Position relative to the button's container, not the entire card
          var $buttonContainer = $this.closest('.product-card-cart, .product-form__item--submit, .product-card__overlay');
          if ($buttonContainer.length === 0) {
            $buttonContainer = $this.parent();
          }
          $buttonContainer.css('position', 'relative');
          $buttonContainer.append(selectorHtml);
          
          // Add z-index helper class to grid item
          var $gridItem = $this.closest('.grid__item');
          $gridItem.addClass('variant-selector-active');
          
          // Store product data on the selector and trigger initial selection
          var $newSelector = $buttonContainer.find('.inline-variant-selector');
          $newSelector.data('product-data', product);
          
          // Trigger initial variant selection
          updateVariantSelection($newSelector, product);
          
          // Show the selector
          $newSelector.fadeIn(300);
          console.log('Showing selector');
        },
        error: function(xhr, status, error) {
          console.log('First AJAX attempt failed:', {
            status: status,
            error: error,
          ...
          var altJsonUrl = '/products/' + productHandle + '.js';
          ...
          $.ajax({
            ...
            timeout: 5000,
            success: function(product) {
              console.log('Product data received (alternative URL):', product);
              ...
              // Position relative to the button's container
              var $buttonContainer = $this.closest('.product-card-cart, .product-form__item--submit, .product-card__overlay');
              if ($buttonContainer.length === 0) {
                $buttonContainer = $this.parent();
              }
            ...
            error: function(xhr2, status2, error2) {
              console.log('Both AJAX attempts failed:', {
                firstAttempt: { status: status, error: error, url: jsonUrl },
                secondAttempt: { status: status2, error: error2, url: altJsonUrl }
              });
              
              // Remove loading state
              $this.removeClass('is-loading');
              
              // Fallback to product page if both attempts fail
              window.location.href = productUrl;
            }
          });
        }
...
function trigger_change(){
  setTimeout(function(){
       ...
       document.querySelectorAll('.custom_variant').forEach(function(el){
  ...
  setTimeout(function(){  _this.classList.add('selected');},200);
    // get value attribute from clicked div
    let value = this.getAttribute('value');

    // find the select dropdown
    let select = this.closest('.variant-option-group')
                     .querySelector('.variant-option-select');

    // set select value
    select.value = value;

    // trigger change event (important for Shopify variant change)
...
// Choose Options popup: any variant value that starts with "Warm Weather" (e.g. "Warm Weather (60–100°F)")
theme._isWarmWeatherVariantButton = function(optionValue) {
  var v = (optionValue || '').trim().toLowerCase();
  return v === 'warm weather' || v.indexOf('warm weather') === 0;
};

// Inline variant selector creator
theme.createInlineVariantSelector = function(product) {
  console.log('Creating selector for product:', product);
  
  try {
    var html = '<div class="inline-variant-selector" style="display: none;">';
    html += '<div class="variant-selector-content">';
    html += '<div class="variant-selector-header">';
    html += '<h4>Choose Options</h4>';
    html += '<button class="variant-selector-close" type="button">&times;</button>';
    html += '</div>';
    ...
    if (product.options && product.options.length > 0) {
      ...
      for (var i = 0; i < product.options.length; i++) {
        ...
        if(option_name == 'Environment' || option_name == 'Temperature' || option_name == 'Envionment') option_name = 'Select the temperature range you are shooting in';
        html += '<label class="variant-option-label">' + option_name + ':</label>';
        html += '<select class="variant-option-select" data-option-index="' + i + '"  style="display:none;">';
        
        for (var j = 0; j < option.values.length; j++) {
          html += '<option value="' + option.values[j] + '">' + option.values[j] + '</option>';
        }
        
        html += '</select>';
        html += '<div class="variant-option-select-custom">';
        
        for (var j = 0; j < option.values.length; j++) {
          var optVal = option.values[j];
          var isWarm = theme._isWarmWeatherVariantButton(optVal);
          var baseStyle = 'justify-content: center; align-items: center; display: flex;cursor:pointer;height: 40px; font-size: 12px; border: 1px solid gray; padding: 2px; margin: 0px 0;';
          if (isWarm) {
            html += '<div class="variant-option-warm-wrap">';
            html += '<div class="custom_variant da-inline-warm-weather-variant btn-' + j + '" style="' + baseStyle + '" value="' + optVal + '">' + optVal + '</div>';
            html += '<p class="variant-option-warm-recommend">Recommended for this time of year</p>';
            html += '</div>';
          } else {
            html += '<div class=" custom_variant btn-' + j + '" style="' + baseStyle + '" value="' + optVal + '">' + optVal + '</div>';
          }
        }
      ...
      console.log('No options found for product');
    }
    
    html += '<div class="variant-selector-actions">';
    html += '<button class="btn btn-theme variant-add-to-cart" data-product-id="' + product.id + '">Add to Cart</button>';
    html += '</div>';
    html += '</div>';
    html += '</div>';
    ...
    console.error('Error creating variant selector:', error);
    return '<div class="inline-variant-selector" style="display: none;"><div class="variant-selector-content"><p>Error loading options</p></div></div>';
  }
};
...
// Handle inline variant selector interactions
$(document).on('click', '.variant-selector-close', function() {
  var $selector = $(this).closest('.inline-variant-selector');
  var $gridItem = $selector.closest('.grid__item');
  
  $selector.fadeOut(300, function() {
    // Remove z-index helper class when selector is closed
    $gridItem.removeClass('variant-selector-active');
  });
});

// Function to update variant selection
function updateVariantSelection($selector, product) {
  // Get selected options
  var selectedOptions = [];
  $selector.find('.variant-option-select').each(function() {
  ...
  if (selectedVariant) {
    var $addButton = $selector.find('.variant-add-to-cart');
...
$(document).on('change', '.variant-option-select', function() {
  var $selector = $(this).closest('.inline-variant-selector');
  var productId = $selector.find('.variant-add-to-cart').data('product-id');
  
  // Get the product data that was stored when creating the selector
  var product = $selector.data('product-data');
  if (product) {
    updateVariantSelection($selector, product);
  } else {
    console.error('Product data not found on selector');
  }
});

$(document).on('click', '.variant-add-to-cart', function() {
  var $this = $(this);
  ...
  if(document.querySelector('body').classList.contains('template-product')){
    qty_value = parseInt(document.querySelector('[name="quantity"]').value);
  }
  Shopify.addItem(variantId, qty_value, function(item) {
    var htmlVariant = item.variant_title !== null ? '<i>(' + item.variant_title + ')</i>' : '';
    var styleCart = $('.js-mini-cart').attr("data-cartmini");
    ...
    setTimeout(function() {
      ...
      // Hide the variant selector after successful add
      var $selector = $this.closest('.inline-variant-selector');
      var $gridItem = $selector.closest('.grid__item');
